...
public:
    ...
    // 获取指向根节点的指针
    BPlusNode<T>* getRootPtr(){
        return root_ptr;
    }

    // 获取指向第一个节点的指针
    BPlusNode<T>* getFirstNodePtr(){
    ...
    // 设置指向根节点的指针
    void setRootPtr(BPlusNode<T>* ptr){
        this->root_ptr = ptr;
    }

    // 设置指向第一个节点的指针
    void setFirstNodePtr(BPlusNode<T>* ptr){
        this->first_node_ptr = ptr;
    }
    ...
    // 初始化一棵树
    BPlusTree& initializeBPlusTree(int degree, string file_path_name, int flag, vector<T> v){
    ...
    pair<bool, pair<T, BPlusNode<T>*>> recallSplit(BPlusNode<T>* root, T key){
        ...
        if(root->getFlag() == 1){ // 表明是叶子节点
            if((ptr->getKeysSize() + 1) > max_num){ // 表明当前叶子节点需要分裂
                ...
                return make_pair(true, make_pair(ptr->getKeys()[ptr->getKeysSize() - 1], node));
            ...
            pair<bool, pair<T, BPlusNode<T> *>> res = recallSplit(p->getNodePtrs()[position], key);
            ...
            if(res.first){ // 需要往父节点中插入
                ...
                // node->setFlag(0);
                // node->putIntoNewNode(p);
                // return make_pair(true, make_pair(p->getKeys()[p->getKeysSize() / 2], ptr));
            }else{ // 不需要往父节点中插入
                return make_pair(false, make_pair(key, nullptr));
            }
        }
    ...
    BPlusNode<T>* getPositionOfKey(BPlusNode<T>* root_ptr, T key) {
        while (true)
        {
            if (root_ptr->getFlag() == 1) {// 表明查找算法已经查找到叶子结点了，可以返回指向该结点的指针
                return root_ptr;
            }
            // 表明还没有找到相应的叶子结点，采用二分查找的算法找到要查询的下一个结点的指针所在的下标
            vector<T> v = root_ptr->getKeys();
            int position = binarySearch(v, key, 0);
            vector<BPlusNode<T>*> v_ptr = ((BPlusNodeNoLeaf<T> *) root_ptr)->getNodePtrs();
...
public:
    ...
    // 按照给定的位置插入关键字
    void insertByPos(int pos, T key){
...
public:
    ...
    // 设置指向前一个节点的指针
    void setPre(BPlusNodeLeaf<T>* ptr){
        pre = ptr;
    }

    // 设置指向后一个节点的指针
    void setNext(BPlusNodeLeaf<T>* ptr){
        next = ptr;
    }
...
public:
    ...
    // 获取所有指针
    vector<BPlusNode<T>*> getNodePtrs(){
        return node_ptrs;
    }

    // 往非叶子节点直接插入指针
    void putPtr(BPlusNode<T>* ptr){
        node_ptrs.push_back(ptr);
    }

    // 删除指定位置的指针
    void deletePtrByPos(int pos){
        node_ptrs.erase(node_ptrs.begin() + pos);
    }
    ...
    // 重新resize ptrs
    void resizePtrs(){
        node_ptrs.resize((node_ptrs.size() + 1 ) / 2);
    }

    // 把当前节点的一半数据放到新的节点中
    void putIntoNewNode(BPlusNodeNoLeaf<T>* ptr){
        vector<T> v_keys = ptr->getKeys();
        BPlusNode<T>::keys.assign(v_keys.begin() + v_keys.size()/2 + 1, v_keys.end());
        ptr->resizeKeys();
        ...
        if (child_ptr->getKeysSize() < min_num) { // 子节点需要借或者合并
            if(position != 0) { // 表明有左兄弟
                ...
                if (leftbro_ptr->getKeysSize() == min_num) {
                    ...
                    // 先把右边的节点信息全部拷贝到左边
                    leftbro_ptr->copyRightToLeft(child_ptr, BPlusNode<T>::keys[position - 1]);
                    ...
                    BPlusNode<T>::keys.erase(BPlusNode<T>::keys.begin() + position);
                    BPlusNode<T>::insertByPos(position, leftbro_ptr->getKeys()[leftbro_ptr->getKeysSize() - 1]);
                    // 判断是不是叶子节点
                    if (child_ptr->getFlag() == 0) { // 不是叶子节点
                        ((BPlusNodeNoLeaf<T>*) child_ptr)->putPtr(((BPlusNodeNoLeaf<T>*) leftbro_ptr)->getNodePtrs()[leftbro_ptr->getKeysSize() - 1]);
                        ((BPlusNodeNoLeaf<T>*) leftbro_ptr)->deletePtrByPos(leftbro_ptr->getKeysSize() - 1);
                    }
                }
                ...
                if (rightbro_ptr->getKeysSize() == min_num) {
                    // 右边兄弟借不了，直接合并
                    child_ptr->copyRightToLeft(rightbro_ptr, BPlusNode<T>::keys[position]);
                    BPlusNode<T>::keys.erase(BPlusNode<T>::keys.begin() + position - 1);
                    ...
                    // 判断是不是叶子节点
                    if (child_ptr->getFlag() == 0) { // 不是叶子节点
                        ((BPlusNodeNoLeaf<T>*) child_ptr)->putPtr(((BPlusNodeNoLeaf<T>*) rightbro_ptr)->getNodePtrs()[0]);
    ...
    virtual void copyRightToLeft(BPlusNode<T>* ptr, T key) override{
        ...
        for (auto it = p->getNodePtrs().begin(); it != p->getNodePtrs().end(); it++) {
            putPtr(*it);
        }
    }
};
